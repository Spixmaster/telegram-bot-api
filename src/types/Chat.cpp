...
namespace tgbot
{
	...
	Chat::Chat(const std::string &json)
	{
		...
		if(doc.HasMember("photo"))
			photo = std::make_shared<ChatPhoto>(special_tools::get_json_obj_as_string(doc["photo"]));
		...
		if(doc.HasMember("permissions"))
			permissions = std::make_shared<ChatPermissions>(special_tools::get_json_obj_as_string(doc["permissions"]));
