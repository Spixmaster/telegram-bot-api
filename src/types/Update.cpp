...
namespace tgbot
{
	...
	Update::Update(std::string json)
	{
		...
		if(doc.HasMember("edited_message"))
			edited_message = std::make_shared<Message>(SpecialTools::get_json_obj_as_string(doc["edited_message"]));

		if(doc.HasMember("channel_post"))
			channel_post = std::make_shared<Message>(SpecialTools::get_json_obj_as_string(doc["channel_post"]));

		if(doc.HasMember("edited_channel_post"))
			edited_channel_post = std::make_shared<Message>(SpecialTools::get_json_obj_as_string(doc["edited_channel_post"]));

		if(doc.HasMember("inline_query"))
			inline_query = std::make_shared<InlineQuery>(SpecialTools::get_json_obj_as_string(doc["inline_query"]));

		if(doc.HasMember("chosen_inline_result"))
			chosen_inline_result = std::make_shared<ChosenInlineResult>(SpecialTools::get_json_obj_as_string(doc["chosen_inline_result"]));

		if(doc.HasMember("callback_query"))
			callback_query = std::make_shared<CallbackQuery>(SpecialTools::get_json_obj_as_string(doc["callback_query"]));

		if(doc.HasMember("shipping_query"))
			shipping_query = std::make_shared<ShippingQuery>(SpecialTools::get_json_obj_as_string(doc["shipping_query"]));

		if(doc.HasMember("pre_checkout_query"))
			pre_checkout_query = std::make_shared<PreCheckoutQuery>(SpecialTools::get_json_obj_as_string(doc["pre_checkout_query"]));
