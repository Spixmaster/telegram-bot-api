...
namespace tgbot
{
	InlineQueryResultCachedMpeg4Gif::InlineQueryResultCachedMpeg4Gif() : id(), mpeg4_file_id(), title(), caption(), parse_mode(),
		...
		if(doc.IsObject())
		{
			...
			if(doc.HasMember("mpeg4_file_id"))
			{
				if(doc["mpeg4_file_id"].IsString())
					...
					std::cerr << Messages::field_does_not_contain_string("mpeg4_file_id") << std::endl;
			}
	...
	std::string InlineQueryResultCachedMpeg4Gif::parse_to_json() const noexcept
	{
		std::string json = "{";
