...
namespace tgbot
{
	InlineQueryResultCachedGif::InlineQueryResultCachedGif() : id(), gif_file_id(), title(), caption(), parse_mode(),
	...
	InlineQueryResultCachedGif::InlineQueryResultCachedGif(const std::string &json)
	{
		...
		if(doc.IsObject())
		{
			...
			{
				if(doc["gif_file_id"].IsString())
					gif_file_id = doc["gif_file_id"].GetString();
				else
					std::cerr << Messages::field_does_not_contain_string("gif_file_id") << std::endl;
			}
	...
	std::string InlineQueryResultCachedGif::parse_to_json() const noexcept
	{
		std::string json = "{";
