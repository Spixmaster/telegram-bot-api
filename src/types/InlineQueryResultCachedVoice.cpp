...
namespace tgbot
{
	InlineQueryResultCachedVoice::InlineQueryResultCachedVoice() : id(), voice_file_id(), title(), caption(), parse_mode(), reply_markup(),
		...
		if(doc.IsObject())
		{
			...
			if(doc.HasMember("voice_file_id"))
			{
				if(doc["voice_file_id"].IsString())
					...
					std::cerr << Messages::field_does_not_contain_string("voice_file_id") << std::endl;
			}
