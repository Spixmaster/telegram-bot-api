	...
	EncryptedPassportElement::EncryptedPassportElement(const std::string &json)
	{
		...
		if(doc.HasMember("files"))
			for(std::size_t j = 0; j < doc["files"].GetArray().Size(); ++j)
			{
				...
				files.at(j) = std::make_shared<PassportFile>(special_tools::get_json_obj_as_string(doc["files"][j]));
			}

		if(doc.HasMember("front_side"))
			front_side = std::make_shared<PassportFile>(special_tools::get_json_obj_as_string(doc["front_side"]));

		if(doc.HasMember("reverse_side"))
			reverse_side = std::make_shared<PassportFile>(special_tools::get_json_obj_as_string(doc["reverse_side"]));

		if(doc.HasMember("selfie"))
			selfie = std::make_shared<PassportFile>(special_tools::get_json_obj_as_string(doc["selfie"]));
		...
		if(doc.HasMember("translation"))
			for(std::size_t j = 0; j < doc["translation"].GetArray().Size(); ++j)
			{
				...
				files.at(j) = std::make_shared<PassportFile>(special_tools::get_json_obj_as_string(doc["translation"][j]));
			}
